...
static BOOL bridge_va_range_is_valid(uint32_t va, uint32_t size)
{
    if (va < XBOX_BASE_ADDRESS || va >= XBOX_TOTAL_RAM) return FALSE;
    if (size > XBOX_TOTAL_RAM || va > XBOX_TOTAL_RAM - size) return FALSE;
    return TRUE;
}
...
static void bridge_RtlEqualString(void)
{
    ...
    uint32_t len1 = BRIDGE_MEM16(s1_va + 0);
    uint32_t len2 = BRIDGE_MEM16(s2_va + 0);
    uint32_t buf1_va = BRIDGE_MEM32(s1_va + 4);
    uint32_t buf2_va = BRIDGE_MEM32(s2_va + 4);

    if (len1 != len2 || !buf1_va || !buf2_va) {
        g_eax = 0;
        return;
    }

    for (uint32_t i = 0; i < len1; i++) {
        unsigned char c1 = BRIDGE_MEM8(buf1_va + i);
        unsigned char c2 = BRIDGE_MEM8(buf2_va + i);
        if (case_insensitive) {
...
/* ── KeCancelTimer (ordinal 99) ──────────────────────────── */
static void bridge_KeCancelTimer(void)
{
    (void)STACK_ARG(0);
    g_eax = 1; /* TRUE: treat the emulated timer as canceled. */
}

/* ── KeAlertThread (ordinal 95) ──────────────────────────── */
static void bridge_KeAlertThread(void)
{
    uint32_t thread = STACK_ARG(0);
    uint32_t alert_mode = STACK_ARG(1);
    (void)thread;
    (void)alert_mode;
    g_eax = 0;
}
...
/* ── KeDelayExecutionThread (ordinal 256) ───────────────── */
...
{
    if (bridge_va_range_is_valid(ios_va, 8)) {
...
{
    HANDLE direct = bridge_lookup_handle_value(va);
    if (direct) {
        return direct;
    }
    if (bridge_va_range_is_valid(va, 4)) {
    ...
    ULONG disposition, ULONG options)
{
    ...
    if (!xbox_translate_path(xbox_path, win_path, MAX_PATH)) {
        fprintf(stderr, "  [FILE] NtCreateFile translation failed: %s\n", xbox_path);
        fflush(stderr);
        bridge_write_iostatus(iostatus_va, STATUS_OBJECT_PATH_NOT_FOUND, 0);
        return STATUS_OBJECT_PATH_NOT_FOUND;
    }

    fprintf(stderr, "  [FILE] NtCreateFile request: %s -> %S\n", xbox_path, win_path);
    fflush(stderr);

    /* Access mask translation */
    if (access & 0x10000000) win_access |= GENERIC_ALL;
    if (access & 0x80000000 || access & 0x00020089) win_access |= GENERIC_READ;
    if (access & 0x40000000 || access & 0x00020116) win_access |= GENERIC_WRITE;
...
static void bridge_NtCreateFile(void)
{
    ...
    fprintf(stderr,
            "  [FILE] NtCreateFile args: handle=%08X access=%08X obj=%08X ios=%08X attrs=%08X share=%08X disp=%08X opts=%08X\n",
            handle_va, access, obj_attrs, iostatus, file_attrs, share, disposition, options);
    fflush(stderr);
...
static void bridge_NtReadFile(void)
{
    ...
    /* XAPI ReadFileEx passes its completion routine in arg3 and uses the
     * OVERLAPPED as both APC context and IO_STATUS_BLOCK. */
    uint32_t apc_routine = STACK_ARG(3);
    uint32_t apc_context = STACK_ARG(4);
    ...
    if (g_kernel_call_count <= 200 && apc_routine) {
        uint32_t kind = bridge_va_range_is_valid(apc_context, 0x14) ? BRIDGE_MEM32(apc_context + 0x10) : 0xFFFFFFFFu;
        fprintf(stderr,
                "  [FILE] NtReadFile async handle=%p buf=%08X len=%u off=%08X apc=%08X ctx=%08X kind=%08X\n",
                handle, buffer_va, length, offset_va, apc_routine, apc_context, kind);
        fflush(stderr);
    }
    ...
    if (result || GetLastError() == ERROR_HANDLE_EOF) {
        ...
        if (apc_routine) {
            ...
            uint32_t stream = 0;
            uint32_t kind = bridge_va_range_is_valid(apc_context, 0x14) ? BRIDGE_MEM32(apc_context + 0x10) : 0;
            if (kind == 1 && apc_context >= 0x134) {
                stream = apc_context - 0x134;
            } else if (kind == 2 && apc_context >= 0x154) {
                stream = apc_context - 0x154;
            }
            recomp_func_t fn = recomp_lookup_manual(apc_routine);
            if (!fn) fn = recomp_lookup(apc_routine);
            if (fn) {
                g_esp -= 4; BRIDGE_MEM32(g_esp) = apc_context;
                g_esp -= 4; BRIDGE_MEM32(g_esp) = bytes_read;
                g_esp -= 4; BRIDGE_MEM32(g_esp) = 0;
                g_esp -= 4; BRIDGE_MEM32(g_esp) = 0;
                fn();
            } else if (g_kernel_call_count <= 200) {
                fprintf(stderr, "  [FILE] NtReadFile: APC routine 0x%08X not found\n", apc_routine);
                fflush(stderr);
            }
            g_esp = saved_esp;

            if (bridge_va_range_is_valid(stream, 4)) {
                BRIDGE_MEM32(stream) &= 0xFFFFFFFEu;
            }
        }
...
static void bridge_NtQueryInformationFile(void)
{
    ...
    case 34: { /* FileNetworkOpenInformation (56 bytes) */
        ...
        if (!GetFileInformationByHandle(handle, &fi)) {
            fprintf(stderr, "  [FILE] NtQueryInformationFile class 34 failed handle=%p\n", handle);
            fflush(stderr);
            g_eax = 0xC0000001u; return;
        }
        size = ((LONGLONG)fi.nFileSizeHigh << 32) | fi.nFileSizeLow;
        fprintf(stderr, "  [FILE] NtQueryInformationFile class 34 handle=%p size=%lld\n",
                handle, (long long)size);
        fflush(stderr);
        BRIDGE_MEM32(info_va +  0) = fi.ftCreationTime.dwLowDateTime;
        BRIDGE_MEM32(info_va +  4) = fi.ftCreationTime.dwHighDateTime;
        BRIDGE_MEM32(info_va +  8) = fi.ftLastAccessTime.dwLowDateTime;
...
static void bridge_NtQueryVolumeInformationFile(void)
{
    ...
    case 3: { /* FileFsSizeInformation (24 bytes) */
        if (!bridge_va_range_is_valid(info_va, 24) || length < 24) {
            fprintf(stderr,
                    "  [FILE] NtQueryVolumeInformationFile: invalid output va=0x%08X len=%u class=%u\n",
                    info_va, length, infoclass);
            bridge_write_iostatus(ios_va, 0xC000000Du, 0);
            g_eax = 0xC000000Du; /* STATUS_INVALID_PARAMETER */
            break;
        }
...
static void bridge_NtQueryFullAttributesFile(void)
{
    ...
    if (!GetFileAttributesExW(win_path, GetFileExInfoStandard, &fad)) {
        ...
        fprintf(stderr, "  [FILE] NtQueryFullAttributesFile FAILED: %s -> %S (err=%u)\n",
                xbox_path, win_path, err);
        fflush(stderr);
...
static void bridge_NtQueryDirectoryFile(void)
{
    ...
    if (filename_va) {
        ...
        if (fn_buf && fn_len > 0) {
            ...
            pattern[fn_len] = L'\0';
            swprintf_s(search_path, MAX_PATH, L"%ls\\%ls", clean_path, pattern);
        } else {
            swprintf_s(search_path, MAX_PATH, L"%ls\\*", clean_path);
        }
    } else {
        swprintf_s(search_path, MAX_PATH, L"%ls\\*", clean_path);
    }
...
static int stdcall_args_for_ordinal(ULONG ordinal)
{
    ...
    case  99: return 12;  /* KeDelayExecutionThread(3) in this XBE thunk table */
...
static bridge_func_t bridge_for_ordinal(ULONG ordinal)
{
    ...
    /* Synchronization */
    case  95: return bridge_KeAlertThread;
...
static void kernel_thunk_dispatch(void)
{
    ...
    /* Xbox kernel exports are stdcall-style callees. Preserve the translated
     * callee-saved registers across host bridge code so generated callers can
     * keep object pointers in ebx/esi/edi around kernel/XAPI calls. */
    g_ebx = saved_ebx;
    g_esi = saved_esi;
    g_edi = saved_edi;
